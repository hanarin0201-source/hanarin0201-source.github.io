...
function alphaForGrade99(txt){
 ...
 if(/^(?:\d{2})?A(?:급)?$/.test(t))return .24;
 if(/^(?:\d{2})?B(?:급)?$/.test(t))return .30;
 if(/^(?:\d{2})?C(?:급)?$/.test(t))return .20;
 if(/^(?:\d{2})?D(?:급)?$/.test(t))return .24;
 if(/^(?:\d{2})?E(?:급)?$/.test(t))return .24;
 return .24;
}
...
function tintAllGrades99(root=document){
 [...root.querySelectorAll('.tag')].forEach(tag=>{
  ...
  if(container){container.style.backgroundColor=tint;container.style.transition='background-color .15s ease';container.dataset.gradeTint='99'}
 });
}
...
const oldSettings99=renderSettings;renderSettings=function(){oldSettings99();const b=$('settings');if(b)[...b.querySelectorAll('.meta')].forEach(e=>{if(/콕매치 v9[0-8]/.test(e.textContent||''))e.textContent='콕매치 v99 · 급수별 배경색 3단계 강화'})};
